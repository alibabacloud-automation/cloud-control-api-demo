...
if __name__ == '__main__':
    ...
    request_path = "/api/v1/providers/Aliyun/products/SLB/resources/LoadBalancer"
    # 地域
    regionId = "cn-beijing"
    # 异步操作轮询间隔(s)
    period = 1
    ...
    try:
        # 初始化sdk
        cloud_control_client = LoadBalancerSample.create_client()
        # 创建资源
        resource_id = LoadBalancerSample.create_loadbalancer()
        resource = LoadBalancerSample.get_loadbalancer(resource_id)
        print(resource)
        # 更新资源
        update_success = LoadBalancerSample.update_loadbalancer(resource_id)
        # 查询资源
        resource2 = LoadBalancerSample.get_loadbalancer(resource_id)
        print(resource2)
        # 列举资源
        resources = LoadBalancerSample.list_loadbalancers()
        for item in resources:
            print(item)
        # 删除资源
        delete_success = LoadBalancerSample.delete_loadbalancer(resource_id)
